        ...
        {activeTab === 'products' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-16 bg-[#F9FAFC] p-6 rounded-3xl">
            {products.map((product) => (
              <div
                key={product.id}
                className="bg-white border-2 border-gray-100 p-8 rounded-3xl flex flex-col text-left transition-all duration-300 hover:shadow-lg hover:-translate-y-2 group relative">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <span
                    className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase ${getTagStyles(product.tagType)}`}>
                    {product.tag}
                  </span>
                </div>

                <h3 className="text-xl font-bold text-gray-900 group-hover:text-[#7F27FF] transition-colors">
                  {product.name}
                </h3>
                <p className="text-gray-500 mt-3 text-sm leading-relaxed grow">
                  {product.description}
                </p>
                ...
                <ul className="space-y-3 mb-8">
                  {product.features.map((feature, idx) => (
                    <li
                      ...
                      <FaCheck className="text-[#7F27FF] mr-3 text-xs" />{' '}
                      {feature}
                    </li>
                  ))}
                </ul>
                ...
                <button
                  ...
                  className={`btn w-full rounded-4xl py-4 h-auto font-bold border-none transition-all cursor-pointer ${
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-4">
                  {cart.map((item) => (
                    <div
                      key={item.id}
                      className="flex justify-between items-center p-4 bg-gray-50 rounded-2xl transition-colors border border-gray-50">
                      ...
                      <button
                        ...
                        className="text-[#FF3980] cursor-pointer font-bold text-sm hover:underline transition-all hover:scale-105">
                        Remove
                      </button>
                    </div>
                  ))}
                  ...
                  <div className="flex justify-between items-center pt-8 border-t border-gray-100 mt-6">
                    <span className="text-[#627382] font-medium text-[16px]">
                      Total:
                    </span>
                  ...
                  <button
                    ...
                    className="btn rounded-full bg-linear-to-r from-[#4F39F6] to-[#9514FA] w-full text-white h-14 text-[16px] font-bold border-none mt-4 transform hover:scale-[1.01] transition-transform shadow-lg cursor-pointer">
                    Proceed To Checkout
                  </button>
                </div>
              )}
