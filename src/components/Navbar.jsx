...
const Navbar = ({ cart, setActiveTab, isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <nav className="sticky top-0 bg-white/80 backdrop-blur-md z-100 border-b transition-all duration-300 border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-20 py-4 flex justify-between items-center">
        ...
        <ul className="hidden lg:flex gap-4 xl:gap-6 font-semibold text-gray-600 list-none">
          <li>
            <a
              href="#products"
              className="text-[16px] relative group hover:text-[#7F27FF] transition-colors whitespace-nowrap">
              Products
        ...
        <div className="flex items-center gap-2 md:gap-4 shrink-0">
          <div
            className="relative cursor-pointer transition-transform hover:scale-110 p-2"
            onClick={() => {
              setActiveTab('cart');
              setIsMenuOpen(false);
            }}>
          ...
          <button
            ...
            <div className="relative w-6 h-6 flex items-center justify-center">
              <span
                className={`absolute transition-all duration-300 transform ${isMenuOpen ? 'rotate-90 opacity-0' : 'rotate-0 opacity-100'}`}>
                <FaBars />
              </span>
              <span
                className={`absolute transition-all duration-300 transform ${isMenuOpen ? 'rotate-0 opacity-100' : '-rotate-90 opacity-0'}`}>
                <FaTimes />
              </span>
            </div>
          </button>
      ...
      <div
        className={`absolute top-full left-0 w-full bg-white border-b sm:hidden shadow-xl transition-all duration-300 ease-in-out overflow-hidden ${isMenuOpen ? 'max-h-125 opacity-100' : 'max-h-0 opacity-0'}`}>
        <ul className="flex flex-col p-6 gap-4 font-semibold text-gray-600 list-none">
          <li>
            <a
              href="#products"
              onClick={() => setIsMenuOpen(false)}
              className="block hover:text-[#7F27FF]">
              Products
            </a>
          </li>
          <li>
          ...
          <li className="pt-2 flex flex-col gap-3">
            <button className="w-full border-none btn bg-[#7F27FF] text-white rounded-full py-3 cursor-pointer">
              Login
            </button>
            <button className="w-full border-none btn bg-[#7F27FF] text-white rounded-full py-3 cursor-pointer">
              Get Started
